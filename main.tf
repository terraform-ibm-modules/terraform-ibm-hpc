...
resource "time_sleep" "wait_for_vsi_syncup" {
  count           = var.enable_deployer == false && var.scheduler == "Scale" && var.storage_type != "persistent" && can(regex("^ibm-redhat-8-10-minimal-amd64-.*$", (var.storage_instances[*]["image"])[0])) ? 1 : 0
