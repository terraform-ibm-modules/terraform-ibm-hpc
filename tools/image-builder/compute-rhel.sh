...
useradd lsfadmin
# The IBM tool https://github.com/ibm/detect-secrets detects a potential secret keywork.
# However, the code is safe, it only allows lsfadmin to become sudoers.
...
# The IBM tool https://github.com/ibm/detect-secrets detects a potential secret keywork.
# However, the code is safe, it only detected the word API_KEY but no secrets are exposed.
# Removal of API keys
sed -i "s/^VPC_APIKEY=.*/VPC_APIKEY=/g" "${LSF_CONF_PATH}/resource_connector/ibmcloudgen2/credentials"  # pragma: allowlist secret
sed -i "s/^RESOURCE_RECORDS_APIKEY=.*/RESOURCE_RECORDS_APIKEY=/g" "${LSF_CONF_PATH}/resource_connector/ibmcloudgen2/credentials"  # pragma: allowlist secret
