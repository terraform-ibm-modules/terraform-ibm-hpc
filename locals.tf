...
locals {
  ...
  management_vsi_data   = var.enable_deployer ? [] : flatten(module.landing_zone_vsi[0].management_vsi_data)
  management_nodes      = var.enable_deployer ? [] : (flatten([module.landing_zone_vsi[0].management_vsi_data]))[*]["name"]
  management_private_ip = var.enable_deployer ? [] : (flatten([module.landing_zone_vsi[0].management_vsi_data]))[*]["ipv4_address"]
