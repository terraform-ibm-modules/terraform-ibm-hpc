...
locals {
  ...
  compute_subnet_cidr = var.enable_deployer ? "" : local.static_compute_instance_count > 0 ? jsonencode((data.ibm_is_subnet.existing_compute_subnets[*].ipv4_cidr_block)[0]) : ""
  ...
  # sapphire_rapids_profile_check  = strcontains(var.storage_servers[count.index]["profile"], "3-metal") || strcontains(var.storage_servers[count.index]["profile"], "3d-metal")
