...
locals {
  ...
  compute_nodes_list    = var.enable_deployer ? [] : (length(local.compute_nodes) >= 10 ? [format("%s-[%d:%d]", regex("^(.*?)-\\d+$", local.compute_nodes[0])[0], 1, length(local.compute_nodes))] : local.compute_nodes)
...
locals {
  ...
  da_hpc_repo_tag           = "jay_enhance_lsf_deployer" ###### change it to main in future
