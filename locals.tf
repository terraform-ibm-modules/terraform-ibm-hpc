...
locals {
  ...
  storage_bms_host_entry    = var.scheduler == "Scale" ? { for server in local.raw_bm_storage_servers_dns_record_details : server.ipv4_address => server.name } : {}
  storage_tb_bms_host_entry = var.scheduler == "Scale" ? { for server in local.raw_bm_tie_breaker_dns_record_details : server.ipv4_address => server.name } : {}
  protocol_bms_host_entry   = var.scheduler == "Scale" ? { for server in local.raw_bm_protocol_dns_record_details : server.ipv4_address => server.name } : {}
