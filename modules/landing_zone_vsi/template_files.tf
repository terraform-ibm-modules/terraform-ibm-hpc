...
data "template_file" "client_user_data" {
  ...
  vars = {
    ...
    client_dns_domain          = var.dns_domain_names["client"]
  }
}
...
data "template_file" "protocol_user_data" {
  ...
  vars = {
    ...
    storage_interfaces          = local.vsi_interfaces[0] #var.storage_type == "scratch" ? local.vsi_interfaces[0] : local.bms_interfaces[0]
    protocol_interfaces         = local.vsi_interfaces[1] #var.storage_type == "scratch" ? local.vsi_interfaces[1] : local.bms_interfaces[1]
    storage_dns_domain          = local.enable_storage ? var.dns_domain_names["storage"] : ""
    protocol_dns_domain         = local.enable_protocol ? var.dns_domain_names["protocol"] : ""
    vpc_region                  = var.vpc_region
    resource_group_id           = var.resource_group
