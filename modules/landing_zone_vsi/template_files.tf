data "template_file" "ldap_user_data" {
  ...
  vars = {
    ...
    compute_interfaces          = local.vsi_interfaces[0] #var.storage_type == "scratch" ? local.vsi_interfaces[0] : local.bms_interfaces[0]
...
data "template_file" "client_user_data" {
  ...
  vars = {
    ...
    client_interfaces          = local.vsi_interfaces[0] #var.storage_type == "scratch" ? local.vsi_interfaces[0] : local.bms_interfaces[0]
...
data "template_file" "management_user_data" {
  ...
  vars = {
    ...
    management_interfaces          = local.vsi_interfaces[0] #var.storage_type == "scratch" ? local.vsi_interfaces[0] : local.bms_interfaces[0]
...
data "template_file" "storage_user_data" {
  ...
  vars = {
    ...
    storage_disk_type           = local.vsi_interfaces[0] # var.storage_type == "scratch" ? data.ibm_is_instance_profile.storage[0].disks[0].quantity[0].type : ""
...
data "template_file" "protocol_user_data" {
  ...
  vars = {
    ...
    storage_interfaces          = local.vsi_interfaces[0] #var.storage_type == "scratch" ? local.vsi_interfaces[0] : local.bms_interfaces[0]
    protocol_interfaces         = local.vsi_interfaces[0] #var.storage_type == "scratch" ? local.vsi_interfaces[1] : local.bms_interfaces[1]
    storage_dns_domain          = var.dns_domain_names["storage"]
    protocol_dns_domain         = var.dns_domain_names["protocol"]
    vpc_region                  = var.vpc_region
    resource_group_id           = data.ibm_resource_group.existing_resource_group.id
