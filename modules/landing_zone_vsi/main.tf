...
module "storage_vsi" {
  ...
  create_security_group         = false
  security_group                = null
  image_id                      = var.storage_type != "evaluation" ? (local.scale_storage_image_found_in_map ? local.new_storage_image_id : data.ibm_is_image.storage[0].id) : local.evaluation_image_id
...
module "protocol_vsi" {
  count                         = (local.enable_protocol && var.colocate_protocol_instances == false && local.ces_server_type == false) ? 1 : 0
  source                        = "terraform-ibm-modules/landing-zone-vsi/ibm"
  version                       = "5.4.6"
  vsi_per_subnet                = var.protocol_instances[count.index]["count"]
  create_security_group         = false
  security_group                = null
  image_id                      = var.storage_type != "evaluation" ? (local.scale_storage_image_found_in_map ? local.new_storage_image_id : data.ibm_is_image.protocol[0].id) : local.evaluation_image_id
...
module "afm_vsi" {
  count                         = (local.afm_server_type == false && local.enable_afm) ? 1 : 0
  source                        = "terraform-ibm-modules/landing-zone-vsi/ibm"
  version                       = "5.4.6"
  vsi_per_subnet                = var.afm_instances[count.index]["count"]
  create_security_group         = false
  security_group                = null
  image_id                      = var.storage_type != "evaluation" ? (local.scale_storage_image_found_in_map ? local.new_storage_image_id : data.ibm_is_image.afm[0].id) : local.evaluation_image_id
...
module "storage_baremetal_tie_breaker" {
  ...
  source                      = "../baremetal"
  existing_resource_group     = var.resource_group
  image_id                    = local.scale_storage_image_found_in_map ? local.new_storage_image_id : data.ibm_is_image.baremetal_storage[0].id
  prefix                      = format("%s-%s", local.storage_tie_breaker_node_name, substr(local.protocol_subnets[count.index].id, length(local.protocol_subnets[count.index].id) - 4, 4))
  storage_subnets             = [for subnet in local.storage_subnets : subnet.id]
  storage_ssh_keys            = local.ssh_keys
...
module "protocol_baremetal_server" {
  ...
  source                      = "../baremetal"
  existing_resource_group     = var.resource_group
  image_id                     = local.scale_storage_image_found_in_map ? local.new_storage_image_id : data.ibm_is_image.protocol[0].id
...
module "afm_baremetal_server" {
  ...
  source                        = "../baremetal"
  existing_resource_group       = var.resource_group
  image_id                     = local.scale_storage_image_found_in_map ? local.new_storage_image_id : data.ibm_is_image.afm[0].id
