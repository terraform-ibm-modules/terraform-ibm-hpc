...
module "protocol_vsi" {
  count                         = (length(var.protocol_instances) > 0 && var.colocate_protocol_instances == false && local.ces_server_type == false) ? 1 : 0
  source                        = "terraform-ibm-modules/landing-zone-vsi/ibm"
  version                       = "5.1.20"
...
module "protocol_baremetal_server" {
  count                        = (var.colocate_protocol_instances == false && local.ces_server_type == true) ? 1 : 0
  source                       = "../baremetal"
  existing_resource_group      = var.resource_group
  prefix                       = count.index == 0 ? local.protocol_node_name : format("%s-%s", local.protocol_node_name, count.index)
  storage_subnets              = [for subnet in local.storage_subnets : subnet.id]
