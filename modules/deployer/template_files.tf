...
data "template_file" "deployer_user_data" {
  ...
  vars = {
    ...
    compute_public_key_content  = local.enable_bastion && local.enable_compute ? module.compute_key[0].public_key_content : ""
    compute_private_key_content = local.enable_bastion && local.enable_compute ? module.compute_key[0].private_key_content : ""
