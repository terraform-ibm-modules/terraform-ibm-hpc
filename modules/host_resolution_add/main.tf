      ...
      join("\n", flatten([
        ...
        [
          for host in flatten([
            ...
            values(local.normalize_hosts.storage_tb_hosts),
            values(local.normalize_hosts.storage_mgmnt_hosts)
          ]) : "${host.name} ansible_ssh_private_key_file=${local.storage_private_key} storage_type=scratch scale_protocol_node=${var.enable_protocol}"
        ],
        ...
        [
          ...
          "${host.name} ansible_ssh_private_key_file=${local.storage_private_key} storage_type=scratch scale_protocol_node=true"
        ],
        # Protocol BMS hosts
        [
