...
locals {
  instance_data     = [for key, value in data.ibm_resource_instance.afm_cos_instances : value]
  cos_instance_data = concat(flatten(module.landing_zone[*].cos_data), local.instance_data)
  total_instance = [
...
locals {
  # existing_instance_data = [for key, value in data.ibm_resource_instance.exstng_cos_instances : value]
  # total_existing_instances = setsubtract(([for item in local.cos_instance_data : item]), ([for item in local.existing_instance_data : item]))
