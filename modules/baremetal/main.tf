...
module "static_compute_baremetal" {
  source                = "terraform-ibm-modules/bare-metal-vpc/ibm"
  version               = "1.4.6"
  count                 = var.scheduler == "LSF" ? length(var.static_compute_servers) : 0
  server_count          = var.static_compute_servers[count.index].count
  prefix                = format("%s-%02d", var.prefix, count.index + 1)
  profile               = var.static_compute_servers[count.index].profile
  image_id              = var.image_id
  create_security_group = false
  security_group_ids    = var.security_group_ids
  user_data             = var.user_data
