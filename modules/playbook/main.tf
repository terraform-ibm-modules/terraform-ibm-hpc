...
resource "null_resource" "deploy_host_playbook" {
  ...
  provisioner "local-exec" {
    ...
    command     = "sudo ansible-playbook -f 200 -e 'mgmnt_hosts=${local.mgmnt_hosts}' -e 'comp_hosts=${local.comp_hosts}' -e 'login_hosts=${local.login_hosts}' -e 'domain_name=${var.domain_name}' '${local.deployer_hostentry_playbook_path}'"
  }
...
resource "null_resource" "lsf_host_play" {
  ...
  provisioner "local-exec" {
    interpreter = ["/bin/bash", "-c"]
    command     = "sudo ansible-playbook -f 50 -e 'deployer_host=${local.deployer_host}' -e 'mgmnt_hosts=${local.mgmnt_hosts}' -e 'comp_hosts=${local.comp_hosts}' -e 'login_hosts=${local.login_hosts}' -e 'domain_name=${var.domain_name}' -i ${var.inventory_path} '${local.lsf_hostentry_playbook_path}'"
  }
