...
resource "null_resource" "scale_host_play" {
  provisioner "local-exec" {
    ...
    command     = "sudo ansible-playbook -f 50 -i ${local.scale_all_inventory} -l 'compute' --private-key=${local.compute_private_key} -e @${local.scale_cluster_hosts} -e 'domain_names=${local.dns_names}' ${local.scale_hostentry_playbook_path}"
  }
...
resource "null_resource" "remove_host_entry_play" {
  ...
  provisioner "local-exec" {
    interpreter = ["/bin/bash", "-c"]
    command     = "sudo ansible-playbook -f 50 -i ${local.scale_all_inventory} -l 'compute' --private-key=${local.compute_private_key}  ${local.remove_hostentry_playbooks_path}"
  }
  triggers = {
    build = timestamp()
  }
