...
resource "null_resource" "scale_host_play" {
  ...
  provisioner "local-exec" {
    ...
    command     = "sudo ansible-playbook -f 50 -i ${local.scale_all_inventory} -l 'compute' -e @${local.scale_cluster_hosts} -e 'domain_names=${local.dns_names}' ${local.scale_hostentry_playbook_path}"
  }
