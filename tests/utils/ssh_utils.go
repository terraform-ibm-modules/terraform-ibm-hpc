...
// If any step in the process fails, an error is returned with a descriptive message.
func ConnectToHost(publicHostIP, publicHostName, privateHostIP, privateHostName string) (*ssh.Client, error) {
	// Get the SSH private key file path from the environment variable
	sshFilePath := os.Getenv("SSHFILEPATH")
...
func ConnectionE(t *testing.T, publicHostIP, publicHostName, privateHostIP, privateHostName, command string) (string, error) {
	sshFilePath := os.Getenv("SSHFILEPATH")
...
func ConnectToHostsWithMultipleUsers(publicHostIP, publicHostName, privateHostIP, privateHostName string) (*ssh.Client, *ssh.Client, error, error) {
