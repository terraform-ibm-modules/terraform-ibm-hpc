...
// LoginNodeInstances represents each login node instance.
type LoginNodeInstances struct {
	Profile string `yaml:"profile" json:"profile"`
	Image   string `yaml:"image" json:"image"`
}
...
// LDAPServerNodeInstances represents each ldap node instance.
type LDAPServerNodeInstances struct {
	Profile string `yaml:"profile" json:"profile"`
	Image   string `yaml:"image" json:"image"`
	Count   int    `yaml:"count" json:"count"`
}
...
type Config struct {
	BastionImage                                string                    `yaml:"bastion_image"`
	BastionInstanceProfile                      string                    `yaml:"bastion_instance_profile"`
	...
	DeployerInstanceProfile                     string                    `yaml:"deployer_instance_profile"`
	...
	DnsDomainNames                              DnsDomainNames            `yaml:"dns_domain_names"`
	...
	LdapInstances                               []LDAPServerNodeInstances `yaml:"ldap_instances"`
	...
	LoginInstance                               []LoginNodeInstances      `yaml:"login_instance"`
}
...
func setEnvFromConfig(config *Config) error {
	envVars := map[string]interface{}{
		"BASTION_IMAGE":                       config.BastionImage,
		"BASTION_INSTANCE_PROFILE":            config.BastionInstanceProfile,
		...
		"DEPLOYER_IMAGE":                      config.DeployerImage,
		"DEPLOYER_INSTANCE_PROFILE":           config.DeployerInstanceProfile,
		...
		"DNS_DOMAIN_NAMES":                    config.DnsDomainNames,
		...
		"LDAP_INSTANCES":                      config.LdapInstances,
...
func processSliceConfigs(config *Config, envVars map[string]interface{}) error {
	sliceProcessors := []struct {
		...
		{"MANAGEMENT_INSTANCES", config.ManagementInstances},
		{"LoginNodeInstances", config.LoginInstance},
		{"CUSTOM_FILE_SHARES", config.CustomFileShares},
		{"LDAP_INSTANCES", config.LdapInstances},
	}
