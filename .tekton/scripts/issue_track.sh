    ...
    for file in "$DIRECTORY"/$pattern; do
        if [ -f "$file" ]; then
            if [[ "${file}" == *"negative"* ]]; then
                infra_validation_negative_log_fail_check=$(eval "grep -E -w 'FAIL' $file")
                ...
                infra_validation_log_error_check=$(eval "grep -E -w 'FAIL|Error|ERROR' $file")
...
issue_track() {
    ...
    if [ -d "$DIRECTORY" ]; then
        if [[ "${LOG_FILE_NAME}" == *"negative"* ]]; then
            negative_log_error_check=$(eval "grep -E -w 'FAIL' $DIRECTORY/$LOG_FILE_NAME")
            ...
            log_error_check=$(eval "grep -E -w 'FAIL|Error|ERROR' $DIRECTORY/$LOG_FILE_NAME")
...
display_validation_log() {
    ...
    if [ -d "$DIRECTORY" ]; then
        ...
        if [[ -z "$validation_log_file_check" ]]; then
            ...
            validation_log_error_check=$(eval "grep -E -w 'FAIL|Error|ERROR' $DIRECTORY/logs/$LOG_FILE_NAME")
